export const dataWorkExp = [

    {
        image: "/images/fsoft_logo.png",
        nameCompany: "FPT Software",
        position: "Front-end Developer Intern",
        yearWork: "Jun 2021 - Aug 2021",
        description: "Edited and completed documentation for assigned tasks. Collaborated with the team to develop frontend components using HTML/CSS and JavaScript, ensuring responsive design and meeting sprint deadlines."
    }
]

export const dataEducation = [
    {
        image: "/images/fptu_logo.png",
        nameSchool: "FPT University",
        major: "Software Engineering",
        yearStury: "Sep 2019 - Sep 2023"
    }
]

export const dataSkills = ["HTML", "CSS", "JavaScript", "TypeScript", "Tailwind CSS", "Bootstrap", "ReactJS", "NextJS", "Firebase", "Mongodb", "MySQL", "Vite", "Github", "Postman", "Figma", "Photoshop", "VS Code"]

export const dataProject = [
    {
        image: "/images/pizza.png",
        name: "Pizza Teo",
        dateCreate: "Apr 2026",
        desc: "Full-stack food ordering platform with customer and admin roles. Supports online ordering, real-time order tracking, reward points, multilingual support, and multiple payment methods through an admin management dashboard.",
        tech: ["Next.js 14", "React 18", "Tailwind CSS", "MongoDB", "Pusher", "Stripe/ Paypal/ Zalo/ Momo Sandbox", "Leaflet.js", "Swiper.js", "Framer Motion", "next-intl", "dnd-kit", "Nodemailer"],
        deployLink: "https://pizzateo.vercel.app/",
        srcCode: "https://github.com/TuNguyen229VN/pizza-order-app"
    },
    {
        image: "/images/amazon.png",
        name: "Amazon Clone",
        dateCreate: "Mar 2024",
        desc: "An Amazon-inspired e-commerce application that allows users to browse products, manage shopping carts, complete secure online payments, and view order history. Features authentication, product search, filtering, pagination, and multilingual support.",
        tech: ["ReactJS", "Firebase", "Material UI", "Stripe", "axios", "moment.js", "react-flickity", "react-paginate", "react-photoswipe-gallery", "react-toastify", "react-i18next"],
        deployLink: "https://amazon-clone-tunguyen.vercel.app/",
        srcCode: "https://github.com/TuNguyen229VN/amazon-clone-tunguyen"
    },
    {
        image: "https://raw.githubusercontent.com/TuNguyen229VN/sushi_website/main/preview.png",
        name: "Sushi Website",
        dateCreate: "Nov 2022",
        desc: "A responsive sushi restaurant landing page showcasing menu sections, smooth scrolling, scroll animations, and dark/light mode. Built with a mobile-first approach for an optimized experience across all devices.",
        tech: ["HTML", "CSS", "JavaScript"],
        deployLink: "https://sushi-website-ecru.vercel.app/",
        srcCode: "https://github.com/TuNguyen229VN/sushi_website"
    },
    {
        image: "/images/pokedex.png",
        name: "Pokedex Tu Nguyen",
        dateCreate: "Dec 2021",
        desc: "A Pokédex web application that provides detailed information about Pokémon, including types, evolutions, strengths, weaknesses, and stats. Features search, pagination, and detailed Pokémon information pages.",
        tech: ["ReactJS", "axios", "React Bootstrap", "SweetAlert"],
        deployLink: "https://pokedex-tunguyen.vercel.app/",
        srcCode: "https://github.com/TuNguyen229VN/pokedex-tunguyen"
    }
]

export const images = [
    "/images/pic1.jpg",
    "/images/pic2.jpg",
    "/images/pic7.png",
    "/images/pic3.jpg",
    "/images/pic4.jpg",
    "/images/pic5.jpg",
    "/images/pic6.jpg",
    "/images/pic8.png",
];

export const dataCerti = [
    {
        image: "/images/cert1.png",
        name: "Web Design for Everybody: Basics of Web Development & Coding",
        platform_study: "Coursera",
        date: "March 8, 2020",
        link: "https://www.coursera.org/account/accomplishments/specialization/AGA3U7P9CLDW"
    },
    {
        image: "/images/cert2.jpg",
        name: "Startup Entrepreneurship",
        platform_study: "Coursera",
        date: "July 21, 2020",
        link: "https://www.coursera.org/account/accomplishments/specialization/7GDC8DAMHQYG"
    },
    {
        image: "/images/cert3.png",
        name: "CertNexus Certified Ethical Emerging Technologist",
        platform_study: "Coursera",
        date: "June 1, 2022",
        link: "https://www.coursera.org/account/accomplishments/specialization/XG9AWKRNUT3T"
    },
    {
        image: "/images/cert4.png",
        name: "Modern Application Development with Java on AWS",
        platform_study: "Coursera",
        date: "June 4, 2022",
        link: "https://www.coursera.org/account/accomplishments/specialization/6BEXGMZ87UMG"
    },
    {
        image: "/images/cert5.png",
        name: "Project Management Principles and Practices",
        platform_study: "Coursera",
        date: "January 20, 2022",
        link: "https://www.coursera.org/account/accomplishments/specialization/B3FRJ75YG5SN"
    },
    {
        image: "/images/cert6.jpg",
        name: "Computer Communications",
        platform_study: "Coursera",
        date: "November 7, 2020",
        link: "https://www.coursera.org/account/accomplishments/specialization/SMVQ69Z35U7W"
    }
]